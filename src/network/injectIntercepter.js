...
export function inject(service) {
  ...
  service.interceptors.request.use(
    (config) => {
      ...
      config.headers['loadingTag'] && store.commit('loading/startLoading', config.headers['loadingTag']);
      return config;
    },
    (error) => {
  ...
  service.interceptors.response.use(
    (response) => {
      response.config.headers['loadingTag'] &&
        store.commit('loading/stopLoading', response.config.headers['loadingTag']);
    ...
    (error) => {
      if (error.code === 'ECONNABORTED' && error.message.indexOf('timeout') !== -1) {
        error.config.headers['loadingTag'] && store.commit('loading/stopLoading', error.config.headers['loadingTag']);
...
const errorHandler = (response) => {
  ...
  return message.error(codeMessage[response.status]);
};
